...
def run_track1(
    ...
    class YAQAShampoo(torch.optim.Optimizer):
        """Online YAQA-Shampoo: full-matrix Kronecker preconditioning via Sketch B."""
        ...
        @torch.no_grad()
        def update_factors(self, name: str, H_O: torch.Tensor, H_I: torch.Tensor) -> None:
            clean_name = name.removeprefix("_orig_mod.")
    ...
    # ------------------------------------------------------------------
    # YAQA online Sketch-B hooks (before budget_start)
    # ------------------------------------------------------------------
    _yaqa_hooks: list = []
    if resolved_optimizer_name == "yaqadamw":
        print(f"attaching YAQA online Sketch-B hooks (threshold={yaqa_full_dim_threshold})", flush=True)
        model.train()
