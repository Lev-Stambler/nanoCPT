...
def run_track1(
    ...
    if lowpass:
        from lowpass import LowpassConfig, replace_linear_with_lowpass, make_module_filter
        ...
        lowpass_filter = make_module_filter(lowpass_target_filter)
        replaced_lowpass_modules = replace_linear_with_lowpass(model, lowpass_config, lowpass_filter)
        config["lowpass_replaced_modules"] = len(replaced_lowpass_modules)
        print(
            f"lowpass: replaced {len(replaced_lowpass_modules)} nn.Linear modules "
