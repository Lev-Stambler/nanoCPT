...
def run_track1(
    ...
    if resolved_optimizer_name == "yaqadamw" and yaqa_mode == "static":
        ...
        _yaqa_total_samples: dict[str, int] = {}
        _yaqa_hooks: list = []
        ...
        def _yaqa_backward_hook(name: str):
            def hook(module: torch.nn.Linear, grad_input, grad_output):
                ...
                # Per-sample weight gradient: G_b[b,m,n] = sum_t g[b,t,m] * x[b,t,n]
                G_b = torch.einsum("btm,btn->bmn", g, x).float()
                d_O = G_b.pow(2).sum(dim=(0, 2))  # (out,)
                d_I = G_b.pow(2).sum(dim=(0, 1))  # (in,)
                B = G_b.shape[0]
                if name not in _yaqa_d_O:
                    _yaqa_d_O[name] = torch.zeros(d_O.shape[0], device=d_O.device, dtype=torch.float32)
                    _yaqa_d_I[name] = torch.zeros(d_I.shape[0], device=d_I.device, dtype=torch.float32)
                    _yaqa_total_samples[name] = 0
                _yaqa_d_O[name].add_(d_O)
                _yaqa_d_I[name].add_(d_I)
                _yaqa_total_samples[name] += B
        ...
        for h in _yaqa_hooks:
            h.remove()

        for name in _yaqa_d_O:
            d_O = _yaqa_d_O[name] / _yaqa_total_samples[name]
            d_I = _yaqa_d_I[name] / _yaqa_total_samples[name]
