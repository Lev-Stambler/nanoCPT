...
def run_track1(
    ...
    yaqa_diag_fallback: bool = True,
    yaqa_skip_adamw_sq: bool = False,
    ...
    if lowpass_chunk_size < 0:
        raise ValueError("--lowpass-chunk-size must be non-negative")
    if lowpass_chunk_size > 0:
        if lowpass_projector_kind != "dct":
            raise ValueError("--lowpass-chunk-size currently requires --lowpass-projector-kind dct")
        if seq_len % lowpass_chunk_size != 0:
            raise ValueError("--seq-len must be divisible by --lowpass-chunk-size")
        if lowpass_max_rank > lowpass_chunk_size:
            raise ValueError("--lowpass-max-rank must be <= --lowpass-chunk-size when chunking")
    ...
    # Chunked lowpass must not mix documents inside a projection block.
    pack_align = lowpass_chunk_size if lowpass and lowpass_chunk_size > 1 else 1
    ...
    config = {
        ...
        "yaqa_diag_fallback": bool(yaqa_diag_fallback),
        "yaqa_skip_adamw_sq": bool(yaqa_skip_adamw_sq),
    ...
    if lowpass:
        ...
        print(
            ...
            f"max_rank={lowpass_max_rank}, chunk_size={lowpass_chunk_size}, "
            f"energy={lowpass_activation_energy}, "
    ...
    class YAQAShampoo(torch.optim.Optimizer):
        ...
        def __init__(
            ...
            self.yaqa_diag_fallback = yaqa_diag_fallback
            self.yaqa_skip_adamw_sq = yaqa_skip_adamw_sq
            self._name_to_param: dict[str, torch.nn.Parameter] = {}
            self._global_step = 0
            for name, p in named_params:
                if p.requires_grad:
                    ...
                    if p.dim() == 2:
                        m, n = p.shape
                        state = self.state[p]
                        if max(m, n) <= yaqa_full_dim_threshold:
                            state["H_O"] = torch.eye(m, device=p.device, dtype=torch.float32)
                            state["H_I"] = torch.eye(n, device=p.device, dtype=torch.float32)
                            state["Q_O"] = torch.eye(m, device=p.device, dtype=p.dtype)
                            state["Q_I"] = torch.eye(n, device=p.device, dtype=p.dtype)
                            state["lam_O"] = torch.ones(m, device=p.device, dtype=p.dtype)
                            state["lam_I"] = torch.ones(n, device=p.device, dtype=p.dtype)
                        elif yaqa_diag_fallback:
                            state["diag_H_O"] = torch.ones(m, device=p.device, dtype=torch.float32)
                            state["diag_H_I"] = torch.ones(n, device=p.device, dtype=torch.float32)
        ...
        def update_factors(self, name: str, H_O: torch.Tensor | None, H_I: torch.Tensor | None) -> None:
            ...
            if max(m, n) <= self.yaqa_full_dim_threshold:
                ...
                if H_I is not None:
                    state["H_I"].mul_(beta).add_(H_I.to(state["H_I"].device, dtype=torch.float32), alpha=1 - beta)
            elif self.yaqa_diag_fallback:
                if "diag_H_O" not in state:
                    state["diag_H_O"] = torch.ones(m, device=p.device, dtype=torch.float32)
                    state["diag_H_I"] = torch.ones(n, device=p.device, dtype=torch.float32)
                if H_O is not None:
                    diag = H_O.diagonal(dim1=-2, dim2=-1).to(state["diag_H_O"].device, dtype=torch.float32)
                    state["diag_H_O"].mul_(beta).add_(diag, alpha=1 - beta)
                if H_I is not None:
                    diag = H_I.diagonal(dim1=-2, dim2=-1).to(state["diag_H_I"].device, dtype=torch.float32)
                    state["diag_H_I"].mul_(beta).add_(diag, alpha=1 - beta)
        ...
        def step(self, closure=None):
            ...
            for group in self.param_groups:
                ...
                for p in group["params"]:
                    ...
                    # YAQA full-matrix or diagonal preconditioning
                    is_yaqa_param = False
                    ...
                    elif "diag_H_O" in state and p.dim() == 2:
                        is_yaqa_param = True
                        # Diagonal Shampoo fallback: scale by inverse sqrt of diagonal
                        diag_H_O = state["diag_H_O"].to(p.dtype)
                        diag_H_I = state["diag_H_I"].to(p.dtype)
                        denom = torch.sqrt(diag_H_O.unsqueeze(1) * diag_H_I.unsqueeze(0) + self.yaqa_eps)
                        grad = grad / denom
                    ...
                    effective_beta1 = self.yaqa_beta1 if (is_yaqa_param and p.dim() == 2) else beta1

                    exp_avg.mul_(effective_beta1).add_(grad, alpha=1.0 - effective_beta1)

                    # For YAQA params, optionally skip AdamW's second moment to avoid
                    # double-preconditioning (Codex feedback #3).
                    skip_sq = is_yaqa_param and self.yaqa_skip_adamw_sq
                    if not skip_sq:
                    ...
                    if skip_sq:
                        # Pure preconditioned gradient + momentum + weight decay
                        if wd != 0:
                            p.mul_(1.0 - lr * wd)
    ...
    def make_optimizer():
        ...
        if resolved_optimizer_name in {"muon", "muon8", "normuon", "yaqamuon"}:
            ...
            return optimizers
        if resolved_optimizer_name == "yaqamuon_hybrid":
            # Hybrid: Muon on MLP matrices (large dim), YAQA on attention matrices (small dim), AdamW on tail
            muon_params: list[torch.nn.Parameter] = []
            yaqa_named_params: list[tuple[str, torch.nn.Parameter]] = []
            adamw_params: list[torch.nn.Parameter] = []
            for name, parameter in named_trainable_params:
                clean_name = name.removeprefix("_orig_mod.")
                is_embed_or_head = any(part in clean_name for part in ("embed", "lm_head"))
                if parameter.ndim == 2 and not is_embed_or_head:
                    if max(parameter.shape) > yaqa_full_dim_threshold:
                        muon_params.append(parameter)
                    else:
                        yaqa_named_params.append((name, parameter))
                else:
                    adamw_params.append(parameter)
            optimizers: list[torch.optim.Optimizer] = []
            if muon_params:
            ...
            if adamw_params:
                try:
                    ...
                    print(f"warning: bnb AdamW8bit unavailable for hybrid tail: {exc}", flush=True)
                    adamw = torch.optim.AdamW(
                        adamw_params,
                        lr=resolved_adamw_tail_lr,
                        betas=(0.9, 0.95),
                ...
                optimizers.append(adamw)
            print(
                f"optimizer yaqamuon_hybrid: {len(muon_params)} MLP matrices @ Muon lr={resolved_muon_lr:g}, "
                f"{len(yaqa_named_params)} attention matrices @ YAQA lr={lr:g}, "
                f"{adamw_name}: {len(adamw_params)} tail params @ lr={resolved_adamw_tail_lr:g}",
                flush=True,
            )
            return optimizers
        if resolved_optimizer_name == "yaqadamw":
            return YAQAShampoo(
                ...
                yaqa_use_newton_schulz=yaqa_use_newton_schulz,
                yaqa_newton_schulz_iters=yaqa_newton_schulz_iters,
                yaqa_diag_fallback=yaqa_diag_fallback,
                yaqa_skip_adamw_sq=yaqa_skip_adamw_sq,
            )
        kwargs: dict[str, Any] = {
            "lr": lr,
            "betas": (0.9, 0.95),
    ...
    if lowpass and lowpass_config is not None:
        # Fixed bases (DCT/Hadamard/Haar/random) don't need data calibration —
        # basis rows are deterministic and we use --lowpass-max-rank as
        # the per-layer rank directly.
        if lowpass_projector_kind != "svd":
            from lowpass import LowpassLinear as _LowpassLinear

            n_set = 0
            for module in model.modules():
                if isinstance(module, _LowpassLinear):
                    module.set_transform_ranks(
                        lowpass_max_rank,
                        lowpass_max_rank if lowpass_compress_gradients else 0,
                    )
                    n_set += 1
            print(
                f"lowpass: skipped calibration (fixed basis {lowpass_projector_kind}); "
                f"set rank={lowpass_max_rank} on {n_set} modules",
                flush=True,
            )
        else:
            ...
            print(
                ...
                f"(projector=svd, energy={lowpass_activation_energy})",
                flush=True,
            )
    ...
    # ------------------------------------------------------------------
    _yaqa_hooks: list = []
    if resolved_optimizer_name in {"yaqadamw", "yaqamuon", "yaqamuon_hybrid"}:
